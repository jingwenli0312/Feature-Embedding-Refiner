...
class AttentionModel(nn.Module):
    ...
    def _inner(self, input, embeddings):
        if self.problem.NAME == 'cvrp' or self.problem.NAME == 'pdp':
            batch_size = input['depot'].size(0)
        else:
            batch_size = input.size(0)

        outputs = torch.zeros((batch_size, 0), device=self.device)
        sequences = []
        ...
        while not (self.shrink_size is None and state.all_finished()):
            ...
            chosen_action_prob = log_p[:, 0, :][torch.arange(batch_size), selected]
            outputs = torch.cat((outputs, chosen_action_prob[:, None]), dim=-1)
        ...
        if self.problem.NAME == 'cvrp':
            # print('pi.size', pi.size(), self.MAX_LENGTH[self.graph_size])
            zero_tensor = torch.zeros((pi.size(0),
                                       self.MAX_LENGTH[self.graph_size] - pi.size(-1)), dtype=torch.int64,
                                      device=self.device)

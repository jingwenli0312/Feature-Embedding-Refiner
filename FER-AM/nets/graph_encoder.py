...
class MultiHeadAttention_AM(nn.Module):
    ...
    def forward(self, q, h=None, mask=None):
        ...
        heads = torch.matmul(attn, V)  # [n_heads, bs, gs, key_sim]
...
class MLP(torch.nn.Module):
    ...
    def forward(self, in_):  # [batch_size, embed_dim]
        result = self.ReLU(self.fc1(in_))
        result = self.ReLU(self.fc2(result))
        result = self.fc3(result).squeeze(-1)  # [batch_size]
        return result
...
class ValueDecoder(nn.Module):
    ...
    def forward(self, h_em):  # (bs, gs, embed_dim)
        # get embed feature
...
class EmbeddingNet_Pos(nn.Module):
    ...
    def position_encoding(self, x, enc_pattern, solutions, best_solutions, embedding_dim, device):
        ...
        return torch.gather(position_enc, 1,  # [bs, gs, embed_dim]
                            visited_time.long().expand(batch_size, seq_length, embedding_dim)).clone(), torch.gather(
            position_enc, 1,  # [bs, gs, embed_dim]
    ...
    def forward(self, x, solutions, best_solutions):
        ...
        pos_enc, best_pos_enc = self.position_encoding(x, enc_pattern, solutions, best_solutions, self.embedding_dim,
                                                       self.device)
...
class GAT_Solution(nn.Module):
    def __init__(self, problem, embed_dim, hidden_edge_dim, n_heads, alpha, graph_size):
        ...
        self.attentions = SolutionAttentionLayer(problem, embed_dim, hidden_edge_dim, n_heads, graph_size, alpha=alpha,
                                                 concat=True)
    ...
    def forward(self, node_embed, solution, costs, dist, solution_embed_old):
        ...
        solu_embed_elu, solu_embed = self.attentions(node_embed, solution, costs, dist,
                                                     solution_embed_old)  # (gs, embed_dim)
...
class SolutionAttentionLayer(nn.Module):
    ...
    def forward(self, node_embed, solutions, costs, dist, solution_embed_old):  # node_embed: (bs, gs, embed_dim), dist:(bs, gs, gs)

        gs = node_embed.size(1)
        solutions = torch.stack(solutions, 0)  # (num_solu, bs, seq_len)
        costs = torch.stack(costs, 0)  # (num_solu, bs)
        ...
        if self.problem.NAME == 'tsp' or self.problem.NAME == 'pdp':
            # (num_solu, bs, seq_len)
        ...
        # solution_embed_new = solution_embed
...
#     """
#
#     def __init__(self, problem, embedding_dim, hidden_edge_dim, n_heads, graph_size, alpha, concat=False, negative_slope=0.2):
#         super(SolutionAttentionLayer, self).__init__()
...
#         self.problem = problem
#
#         self.fc_edge = nn.Linear(1, hidden_edge_dim, bias=False)
#         self.a = nn.Parameter(torch.empty(size=(2 * embedding_dim + hidden_edge_dim, 1)))
#         nn.init.xavier_uniform_(self.a.data, gain=1.414)
#
#         # self.leakyrelu = torch.nn.functional.leaky_relu(self.alpha, self.negative_slope)
#         self.leakyrelu = nn.LeakyReLU(self.alpha)
#         self.cell = torch.nn.GRUCell(embedding_dim, embedding_dim, bias=True)
#
#     def forward(self, node_embed, solutions, costs, dist, solution_embed_old):  # node_embed: (bs, gs, embed_dim), dist:(bs, gs, gs)
#         if self.problem.NAME == 'tsp' or self.problem.NAME == 'pdp':
#             # solutions: (num_solu, bs, gs), costs: (num_solu, bs), attention: (bs, gs, gs)
#             solutions = torch.stack(solutions, 0)
#             num_solu, bs, gs = solutions.size()
#             next = torch.cat((solutions[:, :, 1:], solutions[:, :, 0].unsqueeze(-1)), -1)
#             edge_pair = torch.cat((solutions.unsqueeze(-1), next.unsqueeze(-1)), -1)  # (num_solu, bs, gs, 2)
#
#             # edge_attr (num_solu, bs, gs, hidden_edge_dim)
#             edge_attr = self.fc_edge(dist.unsqueeze(0).repeat(num_solu, 1, 1, 1).gather(2, solutions.unsqueeze(-1).expand(num_solu, bs, gs, gs)).gather(3, next.unsqueeze(-1)))
#             a_input = self._prepare_attentional_mechanism_input(node_embed, edge_attr, edge_pair)  # (num_solu, bs, gs, 2*embed_dim+hidden_edge_dim)
#             e = self.leakyrelu(torch.matmul(a_input, self.a).squeeze(-1))  # [num_solu, bs, gs]
#             cost = e / costs.unsqueeze(-1).expand_as(e)  # [num_solu, bs, gs]
#
#             attention = torch.zeros((bs, gs, gs), device=node_embed.device)
#             tmp = torch.zeros((bs, gs, gs), device=node_embed.device)
#             for i in range(num_solu):
#                 tmp.index_put_(tuple(torch.cat((torch.arange(bs, device=node_embed.device).view(bs, 1, 1).expand(bs, gs, 1),
#                                                 edge_pair[i]), -1).view(-1, 3).t()), cost[i].view(-1))
#                 attention += tmp
#                 tmp = tmp * 0
#             del tmp
#
#         else:  # cvrp
#             bs, gs, _ = node_embed.size()
#
#             # max_length = max(pi.size(-1) for pi in solutions)
#             # solutions = [F.pad(pi, (0, max_length - pi.size(-1))) for pi in solutions]
#
#             solutions = torch.stack(solutions, 0)
#             num_solu, _, seq_len = solutions.size()
#             next = torch.cat((solutions[:, :, 1:], solutions[:, :, 0].unsqueeze(-1)), -1)
#             edge_pair = torch.cat((solutions.unsqueeze(-1), next.unsqueeze(-1)), -1)  # (num_solu, bs, seq_len, 2)
#             # edge_attr (num_solu, bs, seq_len, hidden_edge_dim)
#             edge_attr = self.fc_edge(dist.unsqueeze(0).repeat(num_solu, 1, 1, 1).gather(2, solutions[..., None].expand(
#                 *solutions.size(), dist.size(-1))).gather(3, next.unsqueeze(-1)))
#             a_input = self._prepare_attentional_mechanism_input(node_embed, edge_attr,
#                                                                 edge_pair)  # (num_solu, bs, seq_len, 2*embed_dim+hidden_edge_dim)
#
#             e = self.leakyrelu(torch.matmul(a_input, self.a).squeeze(-1))  # [num_solu, bs, seq_len]
#             cost = e / costs.unsqueeze(-1).expand_as(e)  # [num_solu, bs, seq_len]
#             attention = torch.zeros((bs, gs, gs), device=node_embed.device)
#             tmp = torch.zeros((bs, gs, gs), device=node_embed.device)  # n_cus
#
...
#             return solution_embed_new, solution_embed_new
#
#     def _prepare_attentional_mechanism_input(self, node_embed, edge_attr, edge_pair):
#         num_solu, bs, gs, _ = edge_pair.size()  # number of nodes, edge_attr:(num_solu, bs, gs, hidden_edge_dim)
#
#         # (num_solu, bs, gs, embed_dim)
#         node_solu = node_embed.unsqueeze(0).repeat(num_solu, 1, 1, 1).gather(2, edge_pair[:, :, :, 0].unsqueeze(-1).expand(num_solu, bs, -1, self.embed_dim))
#         # node_repeated_alternating = node_embed.repeat(1, gs, 1).view(bs, gs, gs, self.embed_dim)
#         node_next = node_embed.unsqueeze(0).repeat(num_solu, 1, 1, 1).gather(2, edge_pair[:, :, :, 1].unsqueeze(-1).expand(num_solu, bs, -1, self.embed_dim))
#
#         # (num_solu, bs, seq_len, 2 * embed_dim + hidden_ege_dim)
#         all_combinations_matrix = torch.cat((torch.cat((node_solu, node_next), dim=-1), edge_attr), -1)
#

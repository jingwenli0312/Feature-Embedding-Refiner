...
from nets.attention_model import AttentionModel as AttentionModel_AM
...
class Actor(nn.Module):
    ...
    def __init__(self,
        ...
        # frozen params
        # for name, param in self.AM.named_parameters():
        ...
        elif problem.NAME == 'pdp':
            self.step_context_dim = embedding_dim
            self.node_dim = 2
            self.init_embed_depot = nn.Linear(2, embedding_dim)
            self.W_placeholder = nn.Parameter(torch.Tensor(2 * embedding_dim))
            self.W_placeholder.data.uniform_(-1, 1)  # Placeholder should be in range of activations
            self.init_embed_pick = nn.Linear(self.node_dim * 2, embedding_dim)
            self.init_embed_delivery = nn.Linear(self.node_dim, embedding_dim)
        ...
        # ablation study for GNL
        # self.embed_project = nn.Linear(embedding_dim * 2, embedding_dim, bias=False)
    ...
    def forward(self, problem, batch, dist, AM_embedding, solutions, costs, solu_embed=None, AM_embed_new_only=False,
                h_x=None, c_x=None,
                loc_embed_only=False, fixed_action=None, require_entropy=False):
        """
        :param input: (batch_size, graph_size, node_dim) input node features
        :return:
        """

        AM_decoder = self.AM._inner

        if problem.NAME == 'tsp':
            batch_size, graph_size, _ = batch.size()
        if problem.NAME == 'cvrp' or problem.NAME == 'pdp':
            batch_size, graph_size, _ = batch['loc'].size()
            graph_size = graph_size + 1  # depot

        if solu_embed is None:
            solu_embed = self.get_init_hidden()
            solu_embed = solu_embed[None, :].repeat(batch_size * graph_size, 1)  # (bs*gs, embed_dim)

        solu_embed_elu, solu_embed = self.gat_solution(AM_embedding, solutions, costs, dist,
                                                       solu_embed)  # [bs, gs, embed_dim]

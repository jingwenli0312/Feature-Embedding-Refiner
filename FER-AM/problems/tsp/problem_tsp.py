...
class TSP(object):
    ...
    def get_initial_solutions(self, batch_size, dataset, dist):
...
#             self.data = [{
#                 'loc': self.data[i],
#                 'dist': self.calculate_distance(self.data[i])} for i in range(num_samples)]
#
#         else:
#             # Sample points randomly in [0, 1] square
#             self.data = [{
#                 'loc': torch.FloatTensor(size, 2).uniform_(0, 1),
#                 'dist': None} for i in range(num_samples)]
#
#             self.size = len(self.data)
#
#             # calculate distance matrix
#             for i, instance in enumerate(self.data):
#                 self.data[i]['loc'] = instance['loc']
#                 self.data[i]['dist'] = self.calculate_distance(self.data[i]['loc'])
#

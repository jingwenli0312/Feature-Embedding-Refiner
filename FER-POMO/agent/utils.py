...
def validate(problem, agent, val_dataset, tb_logger, epoch, cri):
    ...
    for batch in tqdm(DataLoader(val_dataset, batch_size=opts.eval_batch_size),
        ...
        with torch.no_grad():
            ...
            bv, cost_hist, best_hist, r, rec_history = agent.rollout(problem,  # bv, (bs)
                                                                     batch,
                                                                     opts,
                                                                     epoch,
                                                                     cri,
    ...
    print('best', best_value, best_value.min())
    # np.savetxt('cvrp50_conv.csv', search_history.mean(0).cpu().numpy(), delimiter=',')  # T, converge_curve

...
class CVRP(object):
    ...
    def get_initial_solutions(self, batch_size, dataset, dist, method):

        batch_size = dataset['loc'].size(0)

        def get_solution(methods):
            p_size = self.size
            data = torch.cat((dataset['depot'], dataset['loc']), 1)
        ...
        return get_solution(method).clone()
    ...
    def get_costs(dataset, pi, multi_solu=False):
        if multi_solu:
            ...
            loc_with_depot = torch.cat((dataset['depot'], dataset['loc']), 1)  # (bs, gs, 2)
...
class VRPDataset(Dataset):
    ...
    def __init__(self, opts=None, filename=None, size=50, num_samples=1000000, offset=0, distribution=None):
        ...
        if filename is not None:
            ...
            # self.data = [make_instance(data) for _ in range(10)]
